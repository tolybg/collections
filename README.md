# Coding 101 Methods, Classes and Logic
...
1. Run the `HomeworkTest` test class, either from VS Code or via the Developer Console. If all the tests pass, your code satisfies the requirements. Note that the tests are not exhaustive, so there may still be some issues that are picked up at code review time.
